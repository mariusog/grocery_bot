...
class TestPreviewWalkerLimit:
    ...
    def test_20bot_tail_bots_get_to_walk(self):
        ...
        planner = make_planner(
            bots=bots,
            items=[
                {"id": f"i{i}", "type": f"t{i}", "position": [4 + (i % 6) * 2, 2]} for i in range(6)
            ]
            + [
                {"id": f"p{i}", "type": f"p{i}", "position": [4 + (i % 6) * 2, 6]} for i in range(8)
            ],
            orders=[
        ...
        non_wait = sum(1 for a in planner.actions if a["action"] != "wait")
        assert non_wait >= 14, (
    ...
    def test_10bot_walker_limit_reasonable(self):
        ...
        planner = make_planner(
            bots=bots,
            items=[{"id": f"i{i}", "type": f"t{i}", "position": [4 + i * 2, 2]} for i in range(4)]
            + [{"id": f"p{i}", "type": f"p{i}", "position": [4 + i * 2, 6]} for i in range(6)],
            orders=[
        ...
        non_wait = sum(1 for a in planner.actions if a["action"] != "wait")
        assert non_wait >= 7, f"Only {non_wait}/10 bots active. Too restrictive walker limit."
